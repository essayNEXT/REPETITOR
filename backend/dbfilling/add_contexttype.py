...
def add_contexttype(context_types: Dict[str, str]) -> None:
    """Add new content types in DB."""
    ...
    exist_in_db_and_in_context_types_dict = (
        set_context_types & set_existing_context_names
    )
    ...
    if to_add_to_db:
        ...
        query = ContextType.insert(
            ContextType(name=name, description=context_types[name])
        )
        while to_add_to_db:
            name = to_add_to_db.pop()
