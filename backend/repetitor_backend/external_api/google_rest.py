...
async def translate(
    ...
    async with session.post(url, data=params) as response:
        ...
        translated_text = translation['data']['translations'][0]['translatedText'].lower()

    async with session.post(url, data=params_reverse) as response_reverse:
        translation_reverse = await response_reverse.json()
        translated_reverse = translation_reverse['data']['translations'][0]['translatedText'].lower()

    # print('translated_text:', translated_text)
    # print('translated_reverse:', translated_reverse)

    res = [translated_reverse, source_lng, target_lng] if text == translated_text else [translated_text, target_lng, source_lng]
    ...
    async with session.post(url, data=params_verification) as response_verification:
        ...
        translated_verification = translation_verification['data']['translations'][0]['translatedText'].lower()
